...
public class ThemedChestBlock extends ChestBlock implements Themed, ChestTextureProvider {
    ...
    public SpriteIdentifier getSpriteIdentifier(BlockEntity blockEntity, ChestType type, boolean christmas) {
        Theme theme = this.getTheme();
        ChestDecoratableData<?> data = BuiltinDecoratables.CHEST.getData(theme, ChestDecoratableData.class).orElseThrow();
    ...
    protected ChestDecoratableData.ChestTextureContext createTextureContext(Theme theme, BlockEntity blockEntity, ChestType type, boolean christmas) {
        return new ChestDecoratableData.ChestTextureContext(theme, type, false);
    }
}
