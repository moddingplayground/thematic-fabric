...
public class DefaultDecoratables {
    ...
    public static final Decoratable LADDER = register("%s_ladder", t -> {
        if (t == SUNKEN) return new PublicLadderBlock(
            FabricBlockSettings.copyOf(Blocks.LADDER)
                               .sounds(BlockSoundGroup.CHAIN)
                               .strength(3.5f).requiresTool()
        );
        if (t == MECHANICAL) return new PublicLadderBlock(
            FabricBlockSettings.copyOf(Blocks.LADDER)
                               .sounds(BlockSoundGroup.COPPER)
                               .strength(3.5f).requiresTool()
        );
        return new PublicLadderBlock(FabricBlockSettings.copyOf(Blocks.LADDER));
    ...
    public static final Decoratable BOOKSHELF = register("%s_bookshelf", t -> {
        if (t == RUSTIC) return new Block(
            FabricBlockSettings.copyOf(Blocks.BOOKSHELF)
                               .mapColor(MapColor.SPRUCE_BROWN)
        );
        if (t == SUNKEN) return new Block(
            FabricBlockSettings.copyOf(Blocks.BOOKSHELF)
                               .sounds(BlockSoundGroup.NETHERITE)
                               .strength(3.5f).requiresTool()
        );
        if (t == MECHANICAL) return new Block(
            FabricBlockSettings.copyOf(Blocks.BOOKSHELF)
                               .sounds(BlockSoundGroup.COPPER)
                               .strength(3.5f).requiresTool()
        );
        return new Block(FabricBlockSettings.copyOf(Blocks.BOOKSHELF));
    }, (theme, decoratable, block) -> {
        if (theme == RUSTIC) {
            FuelRegistry fuel = FuelRegistry.INSTANCE;
            fuel.add(block, 300);
